...
signed char lstrncmp(char *str1,char *str2,unsigned char len)
{
  unsigned char cnt;
...
#define ENTRY_LEN 15
signed char F32_getDirStruct(unsigned long cluster,char *strBuf)
{
...
unsigned long F32_getClusterNumber(unsigned long curClstr,char *dirName,unsigned char fileOrDir)
{
  struct dirTab *rootTab;
...
/************************************************************************/
signed char F32_readFile(char *filename,unsigned long curCluster,unsigned int offset,char *dataBuf)
{
  unsigned long cnum,snum,bcount=0;
  signed char resp;

  cnum=F32_getClusterNumber(curCluster,filename,FAT_FILE);
...
signed char F32_writeFile(char *filename,unsigned long curCluster,unsigned long int fileSize,char *data,unsigned char flags)
{
  ...
  clusterNumber=F32_getClusterNumber(curCluster,filename,FAT_FILE);   //Check if file exists
